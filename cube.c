...
void print_wall(t_all_data *data, float wall_height, int starting_x, int starting_y)
{
    ...
    int texture_height = data->news[texture_num]->height;
    ...
    while (i < wall_height)
    {
        int x = starting_x;
        int y = starting_y + i;
        int distance_from_top = y - starting_y;
        offset_y = distance_from_top * texture_height / wall_height;
        mlx_get_color_at(data->news[texture_num]->data, offset_x, offset_y, &color);
...
void init_textures(t_all_data *data)
{
    ...
    while (data->cu_map->news[i])
    {
        ...
        data->news[i]->img = mlx_xpm_file_to_image(data->mlx.connection, data->news[i]->path, &data->news[i]->width, &data->news[i]->height);
        if (!data->news[i]->img)
            exit(1);
        data->news[i]->data.addr = mlx_get_data_addr(data->news[i]->img, &data->news[i]->data.bits_per_pixel, &data->news[i]->data.line_length, &data->news[i]->data.endian);
        j = 0;
        while (res[j])
            free(res[j++]);
        free(res);
