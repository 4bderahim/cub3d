...
void    initial_endpoint(t_all_data *data)
{
    ...
    if (direction == 'N')
    {
        ...
        data->endpoint.y = 0;
        data->player.player_angle_rad = 90 * to_deg;
    }
    else if (direction == 'S')
    {
        data->endpoint.x = player_x;
        data->endpoint.y = data->minimap.height;
        data->player.player_angle_rad = 270 * to_deg;
    ...
    else if (direction == 'E')
    {
        ...
        data->player.player_angle_rad = 180 * to_deg;
    }
    else if (direction == 'W')
    {
        data->endpoint.x = 0;
        data->endpoint.y = player_y;
        data->player.player_angle_rad = 0 * to_deg;
    }
}